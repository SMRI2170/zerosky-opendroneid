...
# Build script for distance_30m_time_series circuit (N=20 pairs).
# Uses pot14 (2^14 = 16384 constraints) to accommodate 20 × Distance30mTimeCore instances.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BUILD_CIRCUIT_BIN="${BUILD_CIRCUIT_BIN:-build-circuit}"

CIRCUIT_STEM="distance_30m_time_series"
ASSET_PREFIX="distance30_time_series"
INPUT_PATH="$ROOT_DIR/zk-distance/samples/sample_input_time_series.json"
...
if [[ ! -f "$CIRCUIT_PATH" ]]; then
  echo "missing circuit: $CIRCUIT_PATH" >&2; exit 1
fi
if [[ ! -f "$INPUT_PATH" ]]; then
  echo "missing input json: $INPUT_PATH" >&2; exit 1
fi
...
echo "[2/8] Preparing Powers of Tau (pot14, 2^14 = 16384 constraints)"
...
echo "[8/8] Copying artifacts to rapidsnark_app"
...
if command -v "$BUILD_CIRCUIT_BIN" >/dev/null 2>&1; then
  ...
  "$BUILD_CIRCUIT_BIN" "$CIRCUIT_PATH" "$APP_ASSET_DIR/${ASSET_PREFIX}.wcd"
else
  echo "Skipping .wcd (build-circuit not found). Set BUILD_CIRCUIT_BIN or install it, then re-run."
fi

echo "Done. Artifacts in $BUILD_DIR and $APP_ASSET_DIR."
